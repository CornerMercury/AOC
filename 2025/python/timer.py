...
def main():
    ...
    parser.add_argument(
        ...
        "--compare",
        nargs=3,
        metavar=("DAY", "FILENAME1", "FILENAME2"),
        help="Compare two files for a specific day.",
    )
    parser.add_argument(
        "-n", type=int, default=1, help="Number of iterations (default: 1)."
    )
    ...
    if args.single or args.singleDefault:
        ...
        print("Worst days")
        print("‾‾‾‾‾‾‾‾‾‾")
        worst_times = sorted(results, key=lambda x: x[1] + x[2], reverse=True)[:3]
