...
def bfs(blueprints, depth, start=((1, 0, 0), (0, 0, 0, 0))):
    ...
    for i in range(1, depth):
        ...
        for node in to_visit:
            ...
            if max_ingredients[0] != node[0][0] and all(x <= y for x, y in zip(blueprint, node[1])):
                new_to_visit.add(
                        (
                            (
                            node[0][0] + 1,
            ...
            if max_ingredients[1] != node[0][1] and all(x <= y for x, y in zip(blueprint, node[1])):
                new_to_visit.add(
                        (
                            (
                            node[0][0],
            ...
            if max_ingredients[2] != node[0][2] and all(x <= y for x, y in zip(blueprint, node[1])):
                new_to_visit.add(
                        (
                            (
                            node[0][0],
...
def main():
    blueprints = get_data()[:3]
    total = 1
    for blueprint in blueprints:
        total *= bfs(blueprint, 32)
